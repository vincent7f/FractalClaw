...
- **base.py**: Channel, Message, ChannelConfig - 统一通道接口
- **telegram.py**: TelegramChannel - Telegram 适配器
- **slack.py**: SlackChannel - Slack 适配器
- **discord.py**: DiscordChannel - Discord 适配器

### 网关模块 (fractalclaw/gateway)
- **server.py**: GatewayServer - WebSocket 和 HTTP 服务器
- **router.py**: MessageRouter - 消息路由

### 工具模块 (fractalclaw/tools)
- **browser.py**: BrowserController - 浏览器控制
- **scheduler.py**: TaskScheduler - 任务调度
- **webhook.py**: WebhookManager - Webhook 管理

### 画布模块 (fractalclaw/canvas)
- **__init__.py**: LiveCanvas - 实时画布系统

### 语音模块 (fractalclaw/voice)
- **__init__.py**: VoiceManager - 语音处理 (STT/TTS)
